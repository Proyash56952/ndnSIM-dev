...
data_file = open('distance_no_adjustment1.csv', 'w')
csv_writer = csv.writer(data_file)
csv_writer.writerow(["Time","Node","X","Y","Speed"])
...
def setSpeedToReachNextWaypoint(node, referencePos, targetPos, targetTime, referenceSpeed):
    ...
    csv_writer.writerow([Simulator.Now().To(Time.S).GetDouble(),node.name,prevPos.x,prevPos.y, referenceSpeed])
...
def runSumoStep():
    ...
    for vehicle in g_traciStepByStep.vehicle.getIDList():
        ...
        unchangedSpeed = g_traciStepByStep.vehicle.getSpeedWithoutTraCI(vehicle)
        
        #print("changed speed is: "+str(speed)+" while unchagned would be: "+str(unchangedSpeed))
