...
int main (int argc, char *argv[])
{
  ...
  int rate = 16;
  CommandLine cmd;
  cmd.AddValue ("rate","data rate in kb/s",rate);
  ...
  std::string tracerFile = std::to_string(rate)+"_tracerFile.tr";
  Ptr<OutputStreamWrapper> stream = ascii.CreateFileStream (tracerFile);
